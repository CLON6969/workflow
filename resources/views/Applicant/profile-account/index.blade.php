...
<div class="container mx-auto px-4 py-10 max-w-6xl">
    ...
    <!-- Profile Update Form -->
    <form id="myForm"  method="POST" action="{{ route('Applicant.profile-account.update') }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
    ...
    <div class="mt-10 bg-white rounded-2xl p-6 shadow-md">
        ...
        <form id="passwordForm" method="POST" action="{{ route('Applicant.profile-account.password.update') }}">
            @csrf
            @method('PUT')
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6 mx-4">
        ...
        <form action="{{ route('Applicant.profile-account.destroy') }}" method="POST" target="_top">
            @csrf
            @method('DELETE')
