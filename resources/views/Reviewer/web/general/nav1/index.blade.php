...
<div class="container-fluid py-4">
    ...
    <a href="{{ route('Reviewer.web.general.nav1.create') }}" class="btn btn-primary mb-3">➕ Add Navigation Item</a>
                ...
                <tbody>
                    @forelse($navItems as $nav)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('Reviewer.web.general.nav1.edit', $nav->id) }}" class="btn btn-sm btn-warning">✏️ Edit</a>
                                <form action="{{ route('Reviewer.web.general.nav1.destroy', $nav->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
