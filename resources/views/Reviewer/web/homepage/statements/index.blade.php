...
<div class="container mx-auto px-4 py-8 max-w-6xl">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        ...
        <a href="{{ route('Reviewer.web.homepage.statements.create') }}"
                    ...
                    @foreach($statements as $s)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-center space-x-2">
                                <a href="{{ route('Reviewer.web.homepage.statements.edit', $s->id) }}"
                                   class="inline-block bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-xs">Edit</a>
                                <form action="{{ route('Reviewer.web.homepage.statements.destroy', $s->id) }}" method="POST" class="inline"
            ...
            @foreach($statements as $s)
                <div class="border rounded-lg shadow-sm p-4 bg-white">
                    ...
                    <div class="mt-3 flex space-x-2">
                        ...
                        <form action="{{ route('Reviewer.web.homepage.statements.destroy', $s->id) }}" method="POST" class="flex-1"
                              onsubmit="return confirm('Delete this statement?')">
                            @csrf @method('DELETE')
